...
class WOODMART_License {
    ...
    // Só processa se foi enviado o formulário de ativação
    if ( isset( $_POST['woodmart-purchase-code'] ) ) {
        $code = isset($_POST['purchase-code']) ? sanitize_text_field($_POST['purchase-code']) : '';
        // Tenta pegar o token do POST ou da opção salva no banco
        $token = isset($_POST['token']) ? sanitize_text_field($_POST['token']) : get_option('woodmart_token');
        $this->activate( $code, $token );
