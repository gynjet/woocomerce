...
if ( ! function_exists( 'woodmart_get_center_coords' ) ) {
	...
	function woodmart_get_center_coords( $coords ) {
		...
		foreach ( $coords as $lnglat ) {
			// Faz a validação dos valores para garantir que são numéricos
			$lat = isset($lnglat['lat']) && is_numeric($lnglat['lat']) ? floatval($lnglat['lat']) : 0.0;
			$lon = isset($lnglat['lng']) && is_numeric($lnglat['lng']) ? floatval($lnglat['lng']) : 0.0;
